...
TCliente cadastrar(char *senhaCadastro)
{
	TCliente cliente;
	int aux;	
	
	aux = strlen(senhaCadastro);
	char nome[15];
	int um;
	int valor=33;
	for(int e=0;e<aux;e++)
	...
	strcpy(cliente.sal,nome);
	
	strcat(senhaCadastro, cliente.sal);
...
void lerValorUsuario(TElementoUsuario *novoUsuario, int contador)
{
	...
	char auxSenha[15], auxConfirmarSenha[15];
	printf("\n Digite uma senha: ");
	fflush(stdin);
	gets(auxSenha);
	novoUsuario->senhaUsuario = cadastrar(auxSenha);
	do
	{
		printf("\n Confirme a senha digitada: ");
		fflush(stdin);
		gets(auxConfirmarSenha);
	}while(!validarSenha(auxSenha, auxConfirmarSenha));
...
void lerValorOng(TElementoOng *novaOng, int contador)
{
	...
	fflush(stdin);
	gets(auxSenha);
	novaOng->senhaOng = cadastrar(auxSenha);
	do
	{
		printf("\n Confirme a senha digitada: ");
		fflush(stdin);
...
TNoUsuario* inserirUsuario (TNoUsuario *raizUsuario, TElementoUsuario *novoUsuario)
{
	...
	novoNo->Usuario.senhaUsuario = novoUsuario->senhaUsuario;
	if (raizUsuario == NULL)
	{
		raizUsuario = novoNo;
	}
...
TNoOng* inserirOng (TNoOng *raizOng, TElementoOng *novaOng)
{
	...
	novoNo->Ong.senhaOng = novaOng->senhaOng;
	for(int aux=0; aux<=9; aux++)
	{
